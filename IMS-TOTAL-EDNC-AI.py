...
def generate_ai_report(defect_results, optimized_params, num_actions=3, lang="ko", is_optimized=True):
    if not GEMINI_API_KEY:
        return ("⚠️ GEMINI_API_KEY가 설정되지 않았습니다. "
                "Streamlit Cloud의 App settings → Secrets에 GEMINI_API_KEY를 등록한 뒤 다시 시도해주세요.")
    try:
...
반드시 한국어로만 답하세요. 생각 과정(thinking)은 출력하지 말고 최종 답변만 작성하세요."""
        ...
        response_ko = model_ko.generate_content(
            prompt_ko,
            generation_config=genai.types.GenerationConfig(temperature=0),
        )
        report_ko = response_ko.text
...
{report_ko}"""
        ...
        response_en = model_en.generate_content(
            translate_prompt,
            generation_config=genai.types.GenerationConfig(temperature=0),
        )
        report_en = response_en.text
        return report_en
