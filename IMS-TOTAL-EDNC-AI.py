        ...
        if st.sidebar.button("➕ " + ("추가" if _is_ko_sb else "Add"), key="sb_add_tp"):
            if _new_tp and _new_tp != "nt1234":
                ...
                if _saved_ok:
                    ...
                    _err = st.session_state.get('_temp_pwd_save_error', '')
                    st.sidebar.error(
                        ("Google Sheets 저장 실패 — 비번이 저장되지 않았습니다. 잠시 후 다시 시도해주세요. ("
                         if _is_ko_sb else
                         "Failed to save to Google Sheets — the password was NOT saved. Please try again. (")
                        + _err[:200] + ")"
                    )
            elif _new_tp == "nt1234":
        ...
        if st.session_state.temp_pwd_list:
            ...
            for _tp_k, _tp_v in list(st.session_state.temp_pwd_list.items()):
                ...
                if _rc2.button("🗑️", key=f"sb_del_{_tp_k}"):
                    _removed_backup = st.session_state.temp_pwd_list.pop(_tp_k)
                    if _save_temp_pwds(st.session_state.temp_pwd_list):
                        st.rerun()
                    else:
                        # 저장 실패 시 세션 목록을 롤백하여 화면상 불일치 방지
                        st.session_state.temp_pwd_list[_tp_k] = _removed_backup
                        _err = st.session_state.get('_temp_pwd_save_error', '')
                        st.sidebar.error(
                            ("삭제 저장 실패 — 다시 시도해주세요. (" if _is_ko_sb
                             else "Failed to save deletion — please try again. (") + _err[:200] + ")"
                        )
        else:
