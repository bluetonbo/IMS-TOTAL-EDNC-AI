...
if not st.session_state.authenticated:
    ...
    with center:
        # 로그인 화면 헤더 — 로고 + 타이틀 + 언어선택을 한 줄로 배치
        ...
        _logo_col, _title_col, _lang_col = st.columns([1, 5, 1], vertical_alignment="center")
        with _logo_col:
            st.markdown(_login_logo, unsafe_allow_html=True)
        with _title_col:
            st.markdown(f"<h2 style='text-align: center; margin:0;'>{L['access_title']}</h2>", unsafe_allow_html=True)
        with _lang_col:
            _lang_opt = ["KO", "EN"]
